...
/*
test_render_full_line_no_padding:
Проверяет идеальный случай: данных ровно на одну строку, паддинг не нужен.
test_render_short_line_padding:
Проверяет работу паддинга: когда данных меньше, чем -n, программа должна заполнить пустые колонки пробелами.
test_render_multi_byte_chunk_no_char_section:
Проверяет, что при -g 2 символьная колонка справа не выводится (как требует стандарт).
test_render_spec_example_second_line:
Тестирует сложный пример из задания: вторая строка, где данных мало, и нужно много паддинга (пробелов).
test_render_custom_plain_text:
Проверяет, что кастомный рендерер правильно выводит обычный текст без спецсимволов.
test_render_custom_format_basic:
Проверяет работу спецификаторов (%ix) в кастомном формате (реверс байт внутри группы).
test_render_custom_format_escape_chars:
Проверяет работу спецсимволов (\n, \t, \\) внутри кастомной строки.
test_render_custom_index_and_offset:
Проверяет корректность вывода индекса строки (%i) и смещения (%n).
test_render_custom_chunk_formatting:
