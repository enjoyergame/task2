...
*/
/*
test_reader_basic_full_file:
Проверяет, что ридер читает весь файл целиком и корректно переходит на вторую строку, когда данные превышают chunk_count.
test_reader_offset_and_size_and_group_example:
Полная проверка примера из задания (-o 3 -l 8 -g 3): проверяет правильность пропуска байтов в начале и ограничение размера.
test_reader_chunk_count_splits_lines:
Проверяет "разрезание" данных на строки: если данных много, ридер выдает их порциями согласно -n.
test_reader_invalid_offset:
Проверка безопасности — если пользователь ввел смещение больше размера файла, ридер должен вернуть -1 (ошибка).
*/
#include "unity.h"
#include "reader.h"
#include "hexdump.h"
